...
使用Python开发的网站视频下载工具，支持对网站动态视频资源的爬取与本地保存功能，特别支持M3U8播放列表的TS分片自动合并和getmovie链接的处理。
...
2. **智能视频资源探测**：使用浏览器模拟器加载页面，支持从HTML和JavaScript中提取视频链接，特别支持M3U8播放列表和getmovie链接
3. **多格式视频下载**：支持普通视频文件、M3U8播放列表和getmovie链接的下载
...
8. **本地密钥文件支持**：使用Resources目录中的本地密钥文件解密加密视频，并在解密后自动删除
...
6. **getmovie链接支持**：自动解析getmovie JSON响应，提取M3U8播放列表链接
7. **安全密钥管理**：自动处理加密视频的密钥，确保安全性

## 技术实现

### 🔧 核心技术
- **GUI框架**：PyQt5，构建现代化的用户界面
- **浏览器模拟**：使用Selenium和Chrome浏览器模拟用户行为，支持JavaScript渲染和动态资源加载
- **视频资源识别**：开发视频链接识别算法，支持提取页面中的视频标签、source标签、iframe标签、JavaScript中的视频链接和getmovie链接
- **并行处理**：使用concurrent.futures.ThreadPoolExecutor实现多线程并行下载
- **视频处理**：使用FFmpeg工具合并TS分片为完整的MP4文件
- **依赖管理**：自动检查和安装Chrome浏览器，确保程序正常运行
- **JSON解析**：支持解析getmovie链接返回的JSON数据，提取M3U8播放列表
...
### 🎯 基本操作
1. **输入目标网址**：在"目标网址"输入框中输入视频网站的URL或getmovie链接
...
3. **解密TS文件**：对于加密的TS文件，程序会自动从网络或本地Resources目录获取密钥进行解密
4. **处理getmovie链接**：程序会自动解析getmovie JSON响应，提取M3U8播放列表链接并开始下载
...
3. **权限问题**：确保程序有足够的权限访问保存路径和Resources目录，避免因权限不足导致下载失败
4. **性能优化**：对于大型视频或多个TS分片的情况，下载时间可能较长，请耐心等待
5. **密钥安全**：本地密钥文件会在使用后自动删除，确保安全性
...
### 📚 工作流程
1. **浏览器模拟**：使用Selenium和Chrome浏览器模拟用户行为，获取页面内容和网络请求
2. **视频资源探测**：解析HTML结构、JavaScript代码和网络请求，提取视频链接、M3U8播放列表和getmovie链接
3. **视频下载**：
   - 对于普通视频：直接下载完整文件
   - 对于M3U8播放列表：解析播放列表，下载所有TS分片
   - 对于getmovie链接：解析JSON响应，提取M3U8播放列表链接
4. **TS分片处理**：
   - 下载所有TS分片
   - 如需解密，使用从网络或本地Resources目录获取的密钥解密
   - 使用FFmpeg合并TS分片为完整的MP4文件
5. **进度监控**：实时更新下载进度，显示分片下载状态
6. **完成处理**：清理临时文件（包括密钥文件），显示下载结果

### 🎯 核心模块
1. **MainWindow**：主窗口类，负责界面管理和用户交互
2. **SyncBrowserSimulator**：浏览器模拟器，负责加载页面和提取视频资源（支持getmovie链接）
